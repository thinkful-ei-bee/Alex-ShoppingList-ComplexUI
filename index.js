...
function renderShoppingList() {
  ...
  if (STORE.search !== '') {
    filteredItems = filteredItems.filter(item => item.name.includes(STORE.search));
  }
...
function handleSearch() {
  $('#js-shopping-list-search-form').submit(function(event) {
    ...
    findSearchItem(searchItem);
    renderShoppingList();
  });
}

function findSearchItem(searchItem) {
  console.log(`Showing only items with  ${searchItem} in it's name`)
...
function handleEditItemClicked() {
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    ...
    // edit the id with new name
    editName(itemId, newName);
    // render the updated shopping list
    renderShoppingList();
  });
...
function editName(itemId, newName) {
  console.log(`Editing item with id  ${itemId} to give it the name ${name}`)
